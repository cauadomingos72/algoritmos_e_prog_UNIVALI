...
int main() {
  ...
  do{
    ...
    for (lin=0;lin<TAM;lin++) {  // Demonstra na tela a Matriz Jogo, que começa zerada (Se repete para os outros "cases")
    ...
    cin >> lin1 >> col1;
    if (matJ[lin1-1][col1-1]!=0 || ((lin1 > 4 || lin1 < 1) || (col1 > 4 || col1 < 1))){ // Caso o jogador escolha uma posição já descoberta, ou um valor que não condiz com a matriz 4x4, o programa irá pedir para digitar uma posição não descoberta (Se repete para os outros "cases" e para a posição 2)
      while (matJ[lin1-1][col1-1]!=0 || ((lin1 > 4 || lin1 < 1) || (col1 > 4 || col1 < 1))) { // A mensagem irá aparecer até que o jogador escolha uma posição não descoberta (Se repete para os outros "cases" e para a posição 2)
    ...
    system("clear");
    cout << "Rodadas Restantes: " << rodadas << endl;
    for (lin=0;lin<TAM;lin++) {  // Demonstra a Matriz Jogo novamente, porém com a primeira posição revelada (Se repete para os outros "cases" e para a posição 2)
      for (col=0;col<TAM;col++){
        cout << matJ[lin][col] << "\t";
      }
    ...
    if (lin2 == lin1 && col2 == col1) { // O programa irá solicitar uma posição diferente caso ele tenha digitado a mesma da primeira posição (Se repete para os outros "cases")
    ...
    if (val1 == val2){ // Se um par for encontrado ele será inserido na Matriz Jogo (Se repete para os outros "cases")
      cout << "Jogada OK" << endl;
      matJ[lin1-1][col1-1]=matP[lin1-1][col1-1];
      matJ[lin2-1][col2-1]=matP[lin2-1][col2-1];
      pares--;
      if (pares==0){ // Caso todos os pares forem encontrados o jogo será encerrado (Se repete para os outros "cases")
        rodadasJ = 24 - rodadas;
        rodadas=0;
      }
    } else { // Se não for encontrado um par os valores que estavam sendo demonstrados para o jogador serão zerados novamentes (Se repete para os outros "cases")
